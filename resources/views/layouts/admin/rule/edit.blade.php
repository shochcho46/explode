...
@include('layouts.common.message.message')
    ...
    <div class="col-xs-12 col-sm-8 col-md-8 col-lg-8 col-xl-8 ">
        ...
        <form method="POST" id="rule" action="{{ route('rule.update',$rule->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="card elegant-color-dark">
                ...
                <div class="card-body px-lg-5 pt-0">
                    ...
                    <div class="form-group white-text">
                        ...
                        <textarea  type="text" class="form-control" id="editor" name="description" placeholder="News Description"  required minlength="3"> {{ old('description') ?? $rule->description }}</textarea>
                      </div>
                      @if($errors->has('description'))
                      <div class="error text-danger m-2">{{ $errors->first('description') }}</div>
                      @endif

                      <input type="hidden"  name="tournament_id"  value="{{ $rule->tournament_id }}" required  class="form-control">
